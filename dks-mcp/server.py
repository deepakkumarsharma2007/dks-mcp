...
def main(port: int, host: str, transport: Literal["sse", "streamable-http"]) -> int:
    ...
    """
    # is_enabled = setup_app_insights(logger_name="dks_mcp_logger")
    # logger.info(f"Application Insights enabled status: {is_enabled}")
    # AioHttpClientInstrumentor().instrument()
    try:
        settings = ServerSettings(host=host, port=port)
    except ValueError as ex:
        logger.error(f"""Failed to load server settings. Make sure environment variable are set.
        Exception: {ex}")""")
        return 1
    
    
    try:
        """Create and configure the DKS MCP Server application
        """
        ...
        if transport == "sse":
            ...
            return 1
        # if os.environ.get("DKS_ENABLE_APP_LEVEL_TELEMETRY") == "true":
